...
DataInputThread::DataInputThread(
    ...
    rtspDisplayThreadId_(INVALID_INSTANCE_ID), batch_(1), framesPerSecond_(framesPerSecond),
    msgNum_(0)
{
...
AclLiteError DataInputThread::Init()
{
    ...
    classifyInferThreadId_ = GetAclLiteThreadIdByName(kClassifyInferName);
...
AclLiteError DataInputThread::ReadPic(shared_ptr<DetectDataMsg> &detectDataMsg)
{
    ...
    ret = dvpp_.JpegD(batchData.decodedImg, dvppImg);
    if (ret == ACLLITE_ERROR) {
        ACLLITE_LOG_ERROR("Pic decode failed");
        return ACLLITE_ERROR;
    }
    batchData.frame = cv::imread(picFile);
    detectDataMsg->batchData.push_back(batchData);
    return ACLLITE_OK;
}
...
AclLiteError DataInputThread::ReadStream(shared_ptr<DetectDataMsg> &detectDataMsg)
{
    ...
    while (realWaitTime_ < waitTime_) {
        ret = cap_->Read(batchData.decodedImg);
        if (ret != ACLLITE_OK) {
    ...
    // get frame
    ImageData yuvImage;
    ret = CopyImageToLocal(yuvImage, batchData.decodedImg, runMode_);
    if (ret == ACLLITE_ERROR) {
    ...
    cv::cvtColor(yuvimg, batchData.frame, CV_YUV2BGR_NV12);
    detectDataMsg->batchData.push_back(batchData);
    lastDecodeTime_ = now;
    return ACLLITE_OK;
}
...
AclLiteError DataInputThread::MsgRead(shared_ptr<DetectDataMsg> &detectDataMsg)
{
    ...
    detectDataMsg->classifyPreThreadId = classifyPreThreadId_;
    detectDataMsg->classifyInferThreadId = classifyInferThreadId_;
    detectDataMsg->classifyPostThreadId = classifyPostThreadId_;
...
AclLiteError DataInputThread::MsgSend(shared_ptr<DetectDataMsg> &detectDataMsg)
{
    ...
    if (detectDataMsg->isLastFrame == false) {
        ...
        for (int i = 0; i < postThreadNum_; i++) {
            cout<<"lastframeNum["<<detectDataMsg->msgNum<<"]"<<endl;
            while (1) {
                ret = SendMessage(detectDataMsg->detectPreThreadId, MSG_PREPROC_DETECTDATA, detectDataMsg);
                if (ret == ACLLITE_ERROR_ENQUEUE) {
                    usleep(kSleepTime);
