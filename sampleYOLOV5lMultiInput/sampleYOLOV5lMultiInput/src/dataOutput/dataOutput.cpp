...
{
    for(int i = 0; i < detectDataMsg->batchData.size(); i++) {
        cv::resize(detectDataMsg->batchData[i].frame, detectDataMsg->batchData[i].frame,
            cv::Size(kOutputWidth, kOutputHeigth),
            0, 0, cv::INTER_LINEAR);
        outputVideo_ << detectDataMsg->batchData[i].frame;
    }
    return ACLLITE_OK;
}
...
AclLiteError DataOutputThread::SaveResultPic(shared_ptr<DetectDataMsg> &detectDataMsg)
{
    for(int i = 0; i < detectDataMsg->batchData.size(); i++) {
        ...
        cv::imwrite(sstream.str(), detectDataMsg->batchData[i].frame);
    }
    return ACLLITE_OK;
}
...
AclLiteError DataOutputThread::PrintResult(shared_ptr<DetectDataMsg> &detectDataMsg)
{
    stringstream sstream;
    sstream.str("");
    sstream << "Channel-" << detectDataMsg->channelId << "-Frame-" << to_string(detectDataMsg->msgNum) << "-result:";
    
    string textPrint = "";
    sstream >> textPrint;
    for (int i = 0; i < detectDataMsg->batchData[0].classifyDataMsg.size(); i++) {

        textPrint = textPrint + "[" + detectDataMsg->batchData[0].classifyDataMsg[i].classify_result
                    + ":" + detectDataMsg->batchData[0].classifyDataMsg[i].detect_result + "]";
    }
    textPrint = textPrint;
    ...
    lastDecodeTime_ = now;
    textPrint = textPrint + "[" + to_string(lastIntervalTime) + "ms]";
    if (!(frameCnt_ % kCountFps)) {
        if (lastRecordTime_ == 0) {
            ...
            lastRecordTime_ = now;
            textPrint = textPrint + "[fps:" + to_string(fps) + "]";
        }
    }
    frameCnt_++;
    cout<<textPrint<<endl;
...
{
    for(int i = 0; i < detectDataMsg->batchData.size(); i++) {
        cv::resize(detectDataMsg->batchData[i].frame, detectDataMsg->batchData[i].frame, cv::Size(kOutputWidth, kOutputHeigth));
        cv::imshow("frame", detectDataMsg->batchData[i].frame);
