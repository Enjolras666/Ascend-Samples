...
AclLiteError DetectPostprocessThread::InferOutputProcess(shared_ptr<DetectDataMsg> detectDataMsg)
{
    ...
    size_t pos = 0;
    for (int n = 0; n < detectDataMsg->batchData.size(); n++) {
        ...
        int srcWidth = detectDataMsg->batchData[n].decodedImg.width;
        int srcHeight = detectDataMsg->batchData[n].decodedImg.height;
        ...
        for (size_t i = 0; i < result.size(); ++i) {
            ...
            ClassifyDataMsg classifyDataMsg;
            classifyDataMsg.lt.x = result[i].x - result[i].width / half;
            classifyDataMsg.lt.y = result[i].y - result[i].height / half;
            classifyDataMsg.rb.x = result[i].x + result[i].width / half;
            classifyDataMsg.rb.y = result[i].y + result[i].height / half;
            className = label[result[i].classIndex] + ":" + to_string(result[i].score);
            // 这里不加标签label 只保留置信度
            // className = to_string(result[i].score);
            classifyDataMsg.detect_result = className;
            // cv::rectangle(detectDataMsg->batchData[n].frame, classifyDataMsg.lt,
            //     classifyDataMsg.rb, kColors[i % kColors.size()], kLineSolid);
            // cv::putText(detectDataMsg->batchData[n].frame, className,
            //     cv::Point(classifyDataMsg.lt.x, classifyDataMsg.lt.y + kLabelOffset),
            //     cv::FONT_HERSHEY_COMPLEX, kFountScale, kFountColor);
            detectDataMsg->batchData[n].classifyDataMsg.push_back(classifyDataMsg);
        }
        free(detectBuff);
        detectBuff = nullptr;
    }
...
{
    while (1) {
        AclLiteError ret = SendMessage(detectDataMsg->classifyPreThreadId, MSG_PREPROC_CLAASSIFYDATA, detectDataMsg);
