...
#include "classifyInference/classifyInference.h"
...
void CreateALLThreadInstance(vector<AclLiteThreadParam>& threadTbl, AclLiteResource& aclDev, std::string inputDataPath)
{
    aclrtRunMode runMode = aclDev.GetRunMode();
    Json::Reader reader;
    Json::Value root;
    ifstream srcFile(inputDataPath, ios::binary);
    if (!srcFile.is_open())
    {
    ...
    if (reader.parse(srcFile, root))
    {
        for (int i = 0; i < root["device_config"].size(); i++)
        {
            ...
            for (int j = 0; j < root["device_config"][i]["model_config"].size(); j++)
            {
                ...
                // Create inferThread
                AclLiteThreadParam classifyInferParam;
                classifyInferParam.threadInst = new ClassifyInferenceThread("../model/InceptionV3_bs8.om");
                classifyInferParam.threadInstName.assign(kClassifyInferName.c_str());
                classifyInferParam.context = context;
                classifyInferParam.runMode = runMode;
                threadTbl.push_back(classifyInferParam);
                for (int k = 0; k < root["device_config"][i]["model_config"][j]["io_info"].size(); k++)
                {
                    ...
                    AclLiteThreadParam classifyPreParam;
                    classifyPreParam.threadInst = new ClassifyPreprocessThread(336, 336, 8);
                    classifyPreParam.threadInstName.assign(classifyPreName.c_str());
                    classifyPreParam.context = context;
                    classifyPreParam.runMode = runMode;
                    classifyPreParam.queueSize = kMsgQueueSize;
                    threadTbl.push_back(classifyPreParam);

                    AclLiteThreadParam classifyPostParam;
                    classifyPostParam.threadInst = new ClassifyPostprocessThread(336, 336,
                        runMode, 8);
                    classifyPostParam.threadInstName.assign(classifyPostName.c_str());
                    classifyPostParam.context = context;
                    classifyPostParam.runMode = runMode;
                    threadTbl.push_back(classifyPostParam);
                    ...
                    dataOutputParam.threadInst = new DataOutputThread(runMode, outputType, outputPath, 1);
...
void StartApp(AclLiteResource& aclDev, std::string inputDataPath)
{
    vector<AclLiteThreadParam> threadTbl;
    CreateALLThreadInstance(threadTbl, aclDev, inputDataPath);
...
int main(int argc, char *argv[])
{
    int argNum = 2;
    if(argc < argNum) {
        ACLLITE_LOG_ERROR("Please input: ./main ../scripts/test.json");
