...
function main()
{
  ...
  ret=`find ${ModelPath} -maxdepth 1 -name yolov5l.om 2> /dev/null`
  if [[ ${ret} ]];then
    echo "[INFO] The yolov5l.om already exists.start buiding"
  else
    echo "[ERROR] yolov5l.om does not exist, please follow the readme to convert the model and place it in the correct position!"
    return 1
  fi
