...
*/
#ifndef CLAASSIFYPREPROCESSTHREAD_H
#define CLAASSIFYPREPROCESSTHREAD_H
#pragma once
#include <iostream>
#include <mutex>
#include <unistd.h>
...
public:
    ClassifyPreprocessThread(uint32_t modelWidth, uint32_t modelHeight,
    uint32_t batch);
...
private:
    ...
    int CopyOneBatchImages(uint8_t* buffer, uint32_t bufferSize,
                           std::vector<ClassifyDataMsg> &detectResultImg, int batchIdx);
    int CopyImageData(uint8_t *buffer,
                      uint32_t bufferSize, ImageData& image);
    AclLiteError Resize(std::vector<ClassifyDataMsg> &detectResultImg);
    AclLiteError Crop(std::vector<ClassifyDataMsg> &detectResultImg, ImageData &orgImg);
